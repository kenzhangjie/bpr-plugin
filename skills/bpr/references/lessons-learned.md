...
**坑四(2026-08-04 真研报实测追加):drop_set 的过滤只看文本、不看位置,会把正文里同形的行也删掉。**
`find_repeated_hf` 判的是「文本跨页重复」,`page_lines` 却拿这个结果去过滤**全文**
所有行——两个条件本该都成立(位置在带内 + 文本重复)才算真页眉页脚,过滤时却只
剩了后一个。实测一份 22 页单栏 PDF:fitz 把项目符号列表的 `•` 切成独立一行,
20 个恰好落在带内、跨 ≥60% 页面出现,`•` 被判成「跨页重复的页眉页脚模式」;
过滤一执行,全文 239 行内容为 `•` 的行被删,其中 219 行来自正文区的真列表项。
**修法**:过滤条件改成「位置落在带内 **且** 文本在 drop_set 里」两者同时成立,
正文区的行永不因这条规则被删。这也顺带收窄了坑二的残留风险(`norm_hf` 把数字
全替 `#`,可能让「每页页脚位置印不同金额的表注」被误删)——修复后该风险只能
影响带内行,影响面大幅缩小。

**坑五(2026-08-04 真研报实测追加):无边框表格,`find_tables()` 直接返回 0 个。**
`page.find_tables()` 依赖框线做表格检测。实测那份 22 页 PDF 封面有一个无边框
表格(靠文字左对齐 + 固定列位置排出的网格),`find_tables()` 一个都没找到,
其单元格文字被拉平混进正文段落流,`collect_tables`/`strip_table_lines` 两处
都对它无感。阶段 2 做图表抽取时不能只依赖 `find_tables()`,无边框表格需要
另想办法(例如靠同一 y 带内多个 x 锚点的文字对齐网格去推断行列边界)。
