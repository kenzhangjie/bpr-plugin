...
**退出码**:`0` 成功 · `2` 输入非法(非 PDF / 已加密)· `3` 需要 OCR(走阶段 3 的 `ocr_pdf.py`)。`3` 覆盖三种情形:整份无文字层、文字层被权限锁、**以及只有个别页无文字层**(图表页/附录页是整页扫描图,研报最常见形态)。最后这种即便 mode 判成 `text` 也照样报错 —— 跳过那几页就是静默丢内容。
...
脚本会报「剔除页眉页脚 N 条模式,实际删除 M 行」「已截断从第 X 页起共 N 页」。**N 是模式条数,M 是真正删掉的行数**,两者含义不同 —— M 明显大于「模式条数 × 页数」时说明有正文被误判成页眉页脚,要人工看 `pages.jsonl`。页数少于 4 时脚本会明说「跳过页眉页脚重复检测(未删任何行)」,此时页眉页脚原样留在正文里,属预期的保守行为。

研报尾部的免责声明该截,但**若截掉的页数明显偏多,加 `--no-truncate` 重跑**再人工判断。
