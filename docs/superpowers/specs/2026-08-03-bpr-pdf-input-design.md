...
> **2026-08-03 实测修正。** 本节初稿写的是"按 block 中心 x 聚类",实测(`fitz` 1.26.5,合成双栏 PDF)证明**该方案不成立**:`get_text("blocks")` 会把同一基线上的左右栏文字**合并进同一个 block**,栏在 block 内部就已经糊在一起,block 级的任何 x 判据都拿不到栏边界——而且 block 级的"通栏"判据还会把所有合并后的正文块误判成通栏。故整条流程下沉到 **line 级**(`get_text("dict")` 的 `blocks[].lines[]`,每行带 bbox)。

三步:

1. **探 gutter**:取正文行(已剔页眉页脚带),滤掉宽度 > 页宽 40% 的**宽行**(它们是通栏标题,会填平真空隙)。剩余行按 `x0` 排序,遍历切分点找最大的"左侧 max(x1) 与右侧 min(x0) 之间的间隙",间隙 ≥ 8pt 且**两侧各有 ≥3 行**则取其中点为 gutter。gutter 必须落在页宽 25%–75% 之间,否则视为单栏。
   - 两侧各 ≥3 行这条约束不能省:少了它,单栏文档正文右侧的大片空白会被当成 gutter。
2. **分类行**:跨 gutter 的行 = 通栏;否则按行中点归左栏 / 右栏。
3. **排序**:以通栏行的 y 为分界把页面切成若干 band,每个 band 内先出左栏(y 序)再出右栏(y 序),通栏行排在其所属 band 之首。

三栏不做(YAGNI)。

已验证的断言(合成 5 页双栏 + 5 页单栏):双栏判定为 2 且单栏判定为 1;通栏标题在最前;左栏全部行先于右栏全部行;页眉页脚被剔净。

### ② 页眉页脚剥离 — 靠跨页重复,不靠 regex

取每页顶部 / 底部各 8% 区域的**行**(与机制 ① 同为 line 级),文本归一化(数字全替为 `#`),**在 ≥60% 页面上重复出现者判为页眉页脚,删**。

已实测通过:5 页合成研报的「中金公司研究部」与「请务必阅读正文之后的免责声明部分 第 # 页 共 # 页」两条被准确识别并剔除。
